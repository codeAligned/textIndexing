...
\begin{Theorem}
  We can search for all occurrences of a string $S$ in $T$ using the suffix array \id{SA} and the Burrwos-Wheeler-Transform \id{BWT} over $T$ in time $\mathcal{O}(m\log \sigma)$, where $m = \vert S \vert$. This is knows as \defi{backward search}{Backward Search}.
\end{Theorem}
